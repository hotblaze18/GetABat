...
export const ProductGroup: FC<{ products: typeof PRODUCTS_CONTENT, selectedProduct: typeof PRODUCTS_CONTENT[number], onChange: (variant: typeof PRODUCTS_CONTENT[number]['variant']) => void }> = ({ products, selectedProduct, onChange }) => {

    const [thumbsSwiper, setThumbsSwiper] = useState(null);

    const groupVariables = useMemo(() => {
        const group = { colors: [] as { name: string, hexRep: string[] }[], sizes: [] as string[], editions: [] as string[] };
        for(const { variant } of products) {
            ...
            if(variant.edition != undefined && !group.editions.includes(variant.edition) && !SubCategoryKeyMap[selectedProduct.category].includes('edition')) {
                        ...
                        <div className={'flex items-center space-x-3'}>
                            ...
                            <div className={'flex flex-wrap items-center gap-3'}>
                                {groupVariables.colors.map(({ name, hexRep }) => (
                                    <button
                                        key={name} className={clsx('flex items-center space-x-2 p-1.5', {
                                    ...
                                    >
                                        <div
                                            ...
                                            style={hexRep.length === 1 ? { background: hexRep[0] } : { backgroundImage: `linear-gradient(45deg, ${hexRep.join(', ')})` }}
                                        />
                            ...
                            <div className={'flex flex-wrap items-center gap-3'}>
                                {
                                    groupVariables.editions.map((edition) => (
                                        <button
                                            key={edition} className={clsx('p-1.5', {
...
export const  Products = () => {
    ...
    const findProductByVariant = (category: string, variant: typeof PRODUCTS_CONTENT[number]['variant']) => {
        ...
        return product || PRODUCTS_CONTENT.find(({ category: pc }) => pc === category) as typeof PRODUCTS_CONTENT[number];
    }
    ...
    const getSubCategoryForProduct = (product: typeof PRODUCTS_CONTENT[number]) => {
        ...
        return capitalize(subCategoryVariantKeys.map((key) => product.variant[key as keyof typeof product.variant] as string).join(' '));
    }
    ...
    return (
        <div className={'space-y-14'}>
            {categories.map((category) => {
                ...
                const productsForSubCategory = productsForCategory.filter((product) => getSubCategoryForProduct(product) === currSelectedSubCategory);
                ...
                return (
                    <div key={category} className={'space-y-4'}>
                        ...
                        <Tabs options={subCategories.map((subCategory) => ({ label: subCategory, value: subCategory }))}
                              value={currSelectedSubCategory} onChange={(value) => setSelectedSubCategory({ ...selectedSubCategory, [category]: value  })} />
                        <ProductGroup products={productsForSubCategory} selectedProduct={selectedProductForSubCategory} onChange={(variant) => {
                            setSelectedProduct({ ...selectedProduct, [currSelectedSubCategory]: findProductByVariant(category, variant) });
                        }} />
                    </div>
                )
            })}
